...
```bash
serverless-video-transcoding$ sam logs -n HelloEfsFunction --stack-name serverless-video-transcoding --tail
```
...
```bash
serverless-video-transcoding$ pip install pytest pytest-mock --user
serverless-video-transcoding$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name serverless-video-transcoding
```
